...
Public Class visitaAT
        Inherits System.Web.UI.Page
        Dim conn As String = ConfigurationManager.ConnectionStrings("connSAG").ConnectionString
    Dim conn2 As String = ConfigurationManager.ConnectionStrings("connSAGv2").ConnectionString
    Dim sentencia, identity As String
        ...
        Me.SqlDataSource1.SelectCommand = "SELECT " & cadena & " FROM `sag_registro_campo_sag_dicta` WHERE 1 = 1  " & c1 & c3 & c4
        ...
        Response.Redirect(String.Format("~/pages/visitaAT.aspx"))
    End Sub
    ...
    Protected Sub GridDatos_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GridDatos.RowCommand
        ...
        If (e.CommandName = "Imprimir") Then
            ...
            stropsres = "SELECT * FROM `sag_registro_campo_sag_dicta` WHERE Id='" & HttpUtility.HtmlDecode(gvrow.Cells(0).Text).ToString & "'   "
            strprod = "SELECT * FROM `sag_registro_campo_sag_dicta_fung`  WHERE Id='" & HttpUtility.HtmlDecode(gvrow.Cells(0).Text).ToString & "'  "
            strops = "SELECT * FROM `sag_registro_campo_sag_dicta_herb` WHERE Id='" & HttpUtility.HtmlDecode(gvrow.Cells(0).Text).ToString & "'  "
            strprodno = "SELECT * FROM `sag_registro_campo_sag_dicta_insec` WHERE Id='" & HttpUtility.HtmlDecode(gvrow.Cells(0).Text).ToString & "'  "


            Dim sdaopsres = New MySqlDataAdapter(stropsres, conn2)
            Dim sdaprod = New MySqlDataAdapter(strprod, conn2)
            Dim sdaops = New MySqlDataAdapter(strops, conn2)
            Dim sdaprodno = New MySqlDataAdapter(strprodno, conn2)

            sdaopsres.Fill(ds, "sag_registro_campo_sag_dicta")
            sdaprod.Fill(ds, "sag_registro_campo_sag_dicta_fung")
            sdaops.Fill(ds, "sag_registro_campo_sag_dicta_herb")
            sdaprodno.Fill(ds, "sag_registro_campo_sag_dicta_insec")


            crystalReport.Load(Server.MapPath("~/pages/registro_visita_campo.rpt"))
            crystalReport.SetDataSource(ds)

            nombre = "Visita de campo" + Today
    ...
    End Sub
        ...
        query = "SELECT * FROM `sag_registro_campo_sag_dicta` ORDER BY departamento, nombre_multiplicador; " +
            "SELECT * FROM `sag_registro_campo_sag_dicta_fung` ORDER BY departamento, nombre_multiplicador; " +
            "SELECT * FROM `sag_registro_campo_sag_dicta_herb` ORDER BY departamento, nombre_multiplicador; " +
            "SELECT * FROM `sag_registro_campo_sag_dicta_insec` ORDER BY departamento, nombre_multiplicador "
            ...
            Using cmd As New MySqlCommand(query)
                Using sda As New MySqlDataAdapter()
                    ...
                    Using ds As New DataSet()
                        ...
                        ds.Tables(0).TableName = "Registro campo"
                        ds.Tables(1).TableName = "Aplicaciones fungicidas"
                        ds.Tables(2).TableName = "Aplicaciones herbicidas"
                        ds.Tables(3).TableName = "Aplicaciones insecticidas"
                        ...
                        Using wb As New XLWorkbook()
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Sag_Registro_campo " & Today & ".xlsx")
